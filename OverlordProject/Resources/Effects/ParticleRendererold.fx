...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float2 texCoord, float4 col, float2x2 uvRotation)
{
	...
	output.Position = mul(float4(pos, 1.0f), gWvp);
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	topLeft = origin + float3(-size/2, size/2, 0);
	topRight = origin + float3(size/2, size/2, 0);
	bottomLeft = origin + float3(-size/2, -size/2, 0);
	bottomRight = origin + float3(size/2, -size/2, 0);
